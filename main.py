...
ax = [None] * 3
fig = [None] * 3
fondo_imagen = None  # Mantener una referencia global a la imagen de fondo
...
def mostrar_nueva_interfaz():
    ...
    global canvas, ax, fig, fondo_imagen  # Declarar como globales para usarlas en la función
    ...
    # Ajustar el tamaño de la ventana nueva para que ocupe toda la pantalla y se posicione en la esquina superior izquierda
    nueva_ventana.geometry(f"{pantalla_ancho}x{pantalla_alto}+0+0")
    nueva_ventana.resizable(False, False)  # Evita que la ventana cambie de tamaño
    ...
    contenedor = tk.Frame(nueva_ventana, bg='red')
    ...
    for i in range(3):
        ...
        canvas_widget.config(width=360, height=260)  # Tamaño fijo de 360x260 píxeles
        canvas_widget.grid(row=0, column=i, padx=10, pady=5)

    # Crear un marco para el botón de cerrar y el botón de volver
    ...
    volver_boton.pack(side='left', padx=10)
...
def restaurar_ventana_principal():
    """Restaura la ventana principal con la imagen de fondo cuando se vuelva a mostrar."""
    global fondo_imagen  # Asegurarse de que la variable global está en uso
    root.deiconify()  # Mostrar la ventana principal
    fondo_label.config(image=fondo_imagen)  # Restaurar la imagen de fondo
